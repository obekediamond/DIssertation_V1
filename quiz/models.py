...
class QuizManager(models.Manager):
    def search(self, query=None):
        ...
        if query is not None:
            ...
            query_set = query_set.filter(or_lookup).distinct() # distinct() is often necessary with Q lookups
        return query_set
...
class Quiz(models.Model):
    cat_q = models.CharField(max_length=20, choices=CAT_OPT, blank=True, verbose_name=_("Cat_q"))
    ...
    timer = models.PositiveIntegerField(verbose_name=_("Timer for each Quiz Question (minutes)"), default=60)
    assignment_start_date = models.DateTimeField(verbose_name=_("Assignment Start Date"), null=True, blank=True)
    assignment_due_date = models.DateTimeField(verbose_name=_("Assignment Due Date"), null=True, blank=True)
    random_order = models.BooleanField(blank=False, default=False, verbose_name=_("Randomize"),
                                       help_text=_("Display the questions in a random order or as they are set?"))
    answers_at_end = models.BooleanField(blank=False, default=False, verbose_name=_("Show answers at the end"),
                                         help_text=_("Answers displayed at the end."))
    exam_paper = models.BooleanField(blank=False, default=False, verbose_name=_("Exam Paper"),
                                     help_text=_("Check to ensure assignment scores are saved"))
    single_attempt = models.BooleanField(blank=False, default=False, verbose_name=_("Single Attempt"),
                                         help_text=_("Single attempt allowed"))
    pass_mark = models.SmallIntegerField(blank=True, default=50, verbose_name=_("Pass Mark"),
                                         validators=[MaxValueValidator(100)],
                                         help_text=_("Minimum pass mark"))
    draft = models.BooleanField(blank=True, default=False, verbose_name=_("Draft"),
                                help_text=_("Quiz only available for lecturers and admin"))
    timestamp = models.DateTimeField(auto_now=True)
    # created_by = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_("User"), on_delete=models.CASCADE)
    objects = QuizManager()

    def get_essay_questions_count(self):
        count = Essay_Question.objects.filter(quiz=self).count()
        return count
...
class SittingManager(models.Manager):
    ...
    def new_sitting(self, user, quiz, course):
        ...
        new_sitting = self.create(user=user, quiz=quiz, course=course, question_order=questions,
                                  question_list=questions, incorrect_questions="", current_score=0, complete=False,
                                  user_answers='{}')
        return new_sitting
...
class Question(models.Model):
    ...
    content = models.CharField(max_length=2000, blank=False,
                               help_text=_("Enter the question text"), verbose_name=_('Question'))
    explanation = models.TextField(max_length=2000, blank=True,
                                   help_text=_("Explanation shown after the question has been answered."),
...
class Essay_Question(Question):
    expected_input = models.TextField(blank=True, help_text=_("Enter the expected input for the code"),
                                      verbose_name=_('Expected Input'))
    expected_output = models.TextField(blank=True, help_text=_("Enter the expected output for the code"),
                                       verbose_name=_('Expected Output'))
...
class MCQuestion(Question):
    ...
    choice_order = models.CharField(max_length=30, null=True, blank=True, choices=CHOICE_ORDER_OPTIONS,
...
class Choice(models.Model):
    ...
    choice = models.CharField(max_length=1000, blank=False,help_text=_("Enter the choice text that you want displayed"),
                              verbose_name=_("Content"))
    correct = models.BooleanField(blank=False, default=False, help_text=_("Is this a correct answer?"),
...
class SampleStudentAnswer(models.Model):
