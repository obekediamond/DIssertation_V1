...
from institution.models import Trimester
from .utils import *
...
class Course(models.Model):
    ...
    trimester = models.CharField(choices=TRIMESTER, max_length=200)
    ...
    @property
    def is_current_trimester(self):
        current_trimester = Trimester.objects.get(is_current_trimester=True)

        if self.trimester == current_trimester.trimester:
            return True
        else:
            return False
...
class CourseAllocation(models.Model):
    # allocated_by = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_("User"), on_delete=models.CASCADE)
    ...
    session = models.ForeignKey("institution.Session", on_delete=models.CASCADE, blank=True, null=True)
...
class Upload(models.Model):
    # uploaded_by = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_("User"), on_delete=models.CASCADE)
...
class UploadVideo(models.Model):
    ...
    slug = models.SlugField(blank=True, unique=True)
    course = models.ForeignKey(Course, on_delete=models.CASCADE)
    video = models.FileField(upload_to='videos/', validators=[FileExtensionValidator(['mp4', 'mkv', 'wmv', '3gp', 'f4v', 'avi', 'mp3'])])
    summary = models.TextField(null=True, blank=True)
